...
\1 Poisson's equation, $\nabla^2 u = Q$, with a specified temperature function on the boundary, $\alpha(x, y)$, is solved by breaking up into the sum of a function where Q = 0 and a function where $\alpha = 0$
	...
	\2 By the one dimension eigenfunctions, $u = \sum_{n = 1}^{\infty} b_n(y) \phi_n(x)$, the series for Q is found as $\sum_{n = 1}^{\infty} (\frac{d^2b_n}{dy^2} - (\frac{n\pi}{L})^2b_n)sin(\frac{n\pi x}{L}) = Q$ by term-by-term differentiation or Green's Theorem
		\3 The $q_n$ coefficients are found as well by the orthogonality of the eigenfunctions
		\3 As a result, $b_n$ solves a 1D nonhomogeneous BVP with homogeneous boundary conditions, found that $b_n(y) = sinh(\frac{n\pi(H - y)}{L})\int_0^y q_n(\tau}sinh(\frac{n\pi\tau}{L})d\tau + sinh(\frac{n\pi y}{L})\int_0^y q_n(\tau}sinh(\frac{n\pi(H - \tau)}{L})d\tau$
	\2 By 2D eigenfunctions, $u = \sum_{m = 1}\sum_{n = 1} b_{mn}sin(\frac{m\pi x}{L})sin(\frac{n\pi y}{H})$, expanded and substituted into the equation to find $Q = \sum\sum -b_{mn}\lambda_{mn}\phi_m(x) \phi_n(y)$
		\3 The eigenfunctions are those found by $\nabla^2 \phi = -\lambda \phi$, such that $\phi_{mn} = sin(\frac{m\pi x}{L})sin(\frac{n\pi y}{H})$, while $\lambda_{mn} = (\frac{m\pi}{L})^2 + (\frac{n\pi}{H})^2$
		\3 As a result, $-b_{mn}\lambda_{mn}$ can be found by the orthogonality of Q with the 2D eigenfunctions with weight 1
	\2 Poisson's equation with nonhomogeneous boundary condition $\alpha(x, y)$ can also be solved directly using the sum of 2D eigenfunctions with constant coefficients
		\3 Since the eigenfunction and u boundary conditions are not the same in this case, it cannot be term-by-term differentiated
		\3 $b_i = \frac{\int\int u\phi_i dxdy}{\int\int \phi_i^2 dxdy} = \frac{-1}{\lambda_i}\frac{\int\int u\nabla^2\phi_i dxdy}{\int\int \phi_i^2 dxdy}$
			\4 This can be simplified by Green's Theorem and the problem specifications, such that $b_i = \frac{-1}{\lambda_i}\frac{\int\int \phi_iQdxdy + \oint \alpha \nabla\phi_i \cdot \vec{n}ds}{\int\int \phi_i^2 dxdy}$
\end{outline*}
...
\1 The solution to the heat equation is able to be simplified by writing it as $u(x, t) = \int^{\infty}_{-\infty}\frac{1}{2\pi}(\int^{\infty}_{-\infty}f(\bar{x})e^{i\omega \bar{x}}d\bar{x})e^{-i\omega x}e^{-k\omega^2 t}d\omega$
	...
	\2 Self-similar solutions are those that are valid under spacial scaling $x = Lx'$ and temporal scaling $t = L^2t'$, such that $u(x, t) = f(x/t^{1/2}) = f(\epsilon)$, where $\epsilon$ is the similarity variable
		\3  
...
\1 For the Laplacian equation in a half-plane, such that it approaches 0 as it goes to infinity in each direction and $u(x, 0) = f(x)$, the Fourier transform can be taken for x, to give an ordinary differential with respect to y
	\2 Since the Fourier transform approaches 0 as well as $y \to \infty$, and the other boundary condition is given by f(x), the homogeneous equation $\pd{^2U}{y^2} - \omega^2U = 0$ can be solved
	\2 By the singularity condition on y, $U(\omega, y) = c(\omega)e^{-|\omega|y}$, where $c(\omega)$ is the Fourier transform of the y = 0 boundary condition
		\3 This is broken up by the convolution theorem, such that $u(x, y) = \int^{\infty}_{-\infty} f(\bar{x})g(x - \bar{x}, y)d\bar$, where $g(x, y) = \int^{\infty}_{-\infty}e^{-|\omega|y}e^{-i\omega x}d\omega = \frac{2y}{x^2 + y^2}$
\end{outline*}
\end{document}
